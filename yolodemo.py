...
#            sys.exit(1)
    ...
    n, c, h, w = net.input_info[input_blob].input_data.shape
    net = ie.read_network(model="yolov5s.xml")

    #cap = cv2.VideoCapture(args.input)
    #frame = cap.read()
    src = cv2.imread("data/images/bus.jpg")
    dst = cv2.resize(src, (640, 640))
    in_frame = letterbox(1, 3, 640, 640)

    exec_net = ie.load_network(network=net, num_requests=0, device_name=args.device)
