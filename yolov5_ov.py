...
##
##               2021.10.02 Masahiro Izutsu
##------------------------------------------
## yolov5_OV.py  (original: yolov5_demo_OV2021.3.py)
##
## 修正箇所 >> 検出したオブジェクトの表示の日本語対応と表示色
...
import myfunction
import color_table80
TEXT_COLOR = color_table80.CR_white
...
#                     min(obj['class_id'] * 7, 255), min(obj['class_id'] * 5, 255))
            ...
            # オブジェクト別の色指定
            BOX_COLOR = color_table80.get_boder_bgr80(obj['class_id'])
            LABEL_BG_COLOR = color_table80.get_back_bgr80(obj['class_id'])
            ...
            cv2.rectangle(frame, (obj['xmin'], obj['ymin']-26), (obj['xmax'], obj['ymin']), LABEL_BG_COLOR, -1)
            cv2.rectangle(frame, (obj['xmin'], obj['ymin']), (obj['xmax'], obj['ymax']), BOX_COLOR, 2)
#            cv2.putText(frame,
#                        det_label + ' ' + str(round(obj['confidence'] * 100, 1)) + ' %',
#                        (obj['xmin'] + 4, obj['ymin'] - 7), cv2.FONT_HERSHEY_COMPLEX, 0.6, TEXT_COLOR, 1)
            myfunction.cv2_putText(img = frame,
                   text = det_label + ' ' + str(round(obj['confidence'] * 100, 1)) + ' %',
                   org = (obj['xmin'] + 4, obj['ymin'] - 7), fontFace = fontPIL,
                   fontScale = 14,
                   color = TEXT_COLOR,
                   mode = 0)
